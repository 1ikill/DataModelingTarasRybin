...
-- for each source table in the northwind_pg database, you need to create a corresponding staging table and load data into it. 
...
--display a sales chart (with the total amount of sales and the quantity of items sold) for the first week of each month.
select 
    month,
...
--display a weekly sales report (with monthly totals) by product category (period: one year). 
select 
    dp.categoryid,
    dc.categoryname,
